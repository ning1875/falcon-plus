...
func QueryExpressions() (ret []*model.Expression, err error) {
	...
	if redi.GetLock(redi.ExpressionCacheLockKey, redi.RedisDisTLockTimeOut) {
		dbm, err := queryExpressions()
		if err != nil {
			return m, err
		}
		redi.SetExpressions2Redis(dbm)
		return dbm, nil
	} else { //没获取到锁 从redis中获取
		now := time.Now()
		redism, err := redi.GetExpressionsFromRedis()
		g.Logger.Infof("QueryExpressions redis cost:%+v", time.Since(now))
		//读取成功返回
		if err == nil {
			return redism, nil
		} else {
