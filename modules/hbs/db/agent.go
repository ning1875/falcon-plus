...
func UpdateAgentNew(agentInfo *model.AgentUpdateInfo) {
	...
	sqlSelect := fmt.Sprintf(
		"select  id  from host where hostname='%s'",
		agentInfo.ReportRequest.Hostname,
	)
	var hostId int64 = -1
	err := DB.QueryRow(sqlSelect).Scan(&hostId)
	if err != nil {
		log.Println("exec_select", sql, "fail", err)
	}
	...
	if hostId <= 0 {
		...
		sql = fmt.Sprintf(
			"update host set ip='%s', agent_version='%s', plugin_version='%s' where id='%d'",
			agentInfo.ReportRequest.IP,
			agentInfo.ReportRequest.AgentVersion,
			agentInfo.ReportRequest.PluginVersion,
			hostId,
	...
	if g.Config().Debug {
		log.Printf("UpdateAgentNew_Exec_sql:%s", sql)
	}
