...
func (t *Hbs) GetOneEndStrategies(req model.SingleEndRpcRequest, reply *model.OneStrategiesResponse) error {
	...
	res := cache.JudgeHostMaintain(req.Endpoint)

	if res == true {
		reply.Strategies = nil
		return nil
	}
	ss := cache.JudgeHostStrategies(req.Endpoint)
	reply.Strategies = ss
	return nil
}
...
// TODO 测试获取单个endpoint 的策略列表，后后续去掉
type OneReq struct {
