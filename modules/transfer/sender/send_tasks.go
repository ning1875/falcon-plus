...
func startSendTasks() {
	...
	polyConcurrent := cfg.Poly.MaxConns
	...
	for node := range cfg.Poly.Cluster {
		queue := PolyQueues[node]
		go forward2PolyTask(queue, node, polyConcurrent)
	}
...
// Graph定时任务, 将 Graph发送缓存中的数据 通过rpc连接池 发送到Graph
func forward2PolyTask(Q *list.SafeListLimited, node string, concurrent int) {
	batch := g.Config().Poly.Batch // 一次发送,最多batch条数据
	addr := g.Config().Poly.Cluster[node]
	sema := nsema.NewSemaphore(concurrent)
	...
	for {
		...
		polyItems := make([]*cmodel.PolyRequest, count)

		for i := 0; i < count; i++ {
			polyItems[i] = items[i].(*cmodel.PolyRequest)
		}

		//log.Printf("polyItems:%+v", polyItems)
		//	同步Call + 有限并发 进行发送
		sema.Acquire()
		go func(addr string, polyItems []*cmodel.PolyRequest, count int) {
			defer sema.Release()
			...
			sendOk := false
			for i := 0; i < 3; i++ { //最多重试3次
				err = PolyConnPools.Call(addr, "Polymetric.Update", polyItems, resp)
				if err == nil {
					sendOk = true
					break
				}
			...
			// statistics
			if !sendOk {
				log.Printf("send poly %s:%s fail: %v", node, addr, err)
				proc.SendToPolyFailCnt.IncrBy(int64(count))
			} else {
				proc.SendToPolyCnt.IncrBy(int64(count))
			}
		}(addr, polyItems, count)
	}
}
