...
func initConnPools() {
	...
	// poly
	polyInstances := nset.NewSafeSet()
	for _, instance := range cfg.Poly.Cluster {
		polyInstances.Add(instance)
	}
	PolyConnPools = backend.CreateSafeRpcConnPools(cfg.Poly.MaxConns, cfg.Poly.MaxIdle,
		cfg.Poly.ConnTimeout, cfg.Poly.CallTimeout, polyInstances.ToSlice())
