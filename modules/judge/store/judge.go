...
func CheckStrategy(L *SafeLinkedList, firstItem *model.JudgeItem, now int64) {
	...
	filterSS := g.CheckNeedJudge(firstItem.Endpoint, firstItem.Metric)
	//strategyMap := g.StrategyMap.Get()
	//strategies, exists := strategyMap[key]
	if len(filterSS) <= 0 {
		return
...
func sendEventProxy(event *model.Event) {

	//g.Logger.Infof("metric:%s Strategy_id:%d UnionStrategyId:%d", event.Metric(), event.Strategy.Id, event.Strategy.UnionStrategyId)
	if event.Strategy.UnionStrategyId > 0 {
		sendEventForUnionStrategy(event)
	} else {
		sendEvent(event)
	}
}

func sendEventForUnionStrategy(event *model.Event) {

	if event.Strategy.UnionStrategyId <= 0 {
		g.Logger.Errorf("[sendEventForUnionStrategy] event.Strategy.UnionStrategyId_le_0 event:%+v", event)
		return
	}
	g.LastEvents.Set(event.Id, event)

	pk := string(event.Strategy.UnionStrategyId)
	node, _ := g.JudgeNodeRing.GetNode(pk)
	addr := g.Config().UnionJudgeS.Cluster[node]
	//g.Logger.Infof("[sendEventForUnionStrategy] addr:%s,event:%+v", addr, event)
	resp := &model.SimpleRpcResponse{}
	sendOk := false
	var err error
	for i := 0; i < 3; i++ { //最多重试3次
		err = g.UnionJudgeConnPools.Call(addr, "Judge.UnionJudge", event, resp)
		if err == nil {
			sendOk = true
			break
		}
	...
	if !sendOk {
		g.Logger.Errorf("[sendEventForUnionStrategy] send to unionJudge %s:%s fail: %+v", node, addr, err)
	}

}

func sendEventToRedis(event *model.Event) {
	//g.Logger.Infof("[报警最终触发] metric:%s,union_id:%d,status:%s num:%d", event.Metric(), event.Strategy.UnionStrategyId, event.Status, event.CurrentStep)
...
func sendEventIfNeed(historyData []*model.HistoryData, isTriggered bool, now int64, event *model.Event, maxStep int) {
	...
	if isTriggered {
		...
		//g.Logger.Infof("isTriggered exists:%+v,lastEvent", lastEvent, exists)
		if !exists || lastEvent.Status[0] == 'O' {
			// 本次触发了阈值，之前没报过警或者上次是恢复状态，得产生一个报警Event
			event.CurrentStep = 1
		...
		// 如果组合报警中的单条恢复了,也要发送过去
		//if event.Strategy.UnionStrategyId > 0 && lastEvent.Status[0] == 'P' {
		//	event.Status = "OK"
		//	event.CurrentStep = 1
		//	sendEventProxy(event)
		//}
