...
func syncUnionStrategies() {
	var strategiesResponse model.UnionStrategiesResponse
	err := g.HbsClient.Call("Hbs.GetUnionStrategies", model.NullRpcRequest{}, &strategiesResponse)
	if err != nil {
		log.Errorf("[ERROR] Hbs.GetUnionStrategies:", err)
		return
	}
	log.Debugf("[syncUnionStrategies] %+v", strategiesResponse.UnionStrategiesMap)
	g.UnionStrategyMap.ReInit(strategiesResponse.UnionStrategiesMap)
}

func rebuildStrategyMap(strategiesResponse *model.StrategiesResponse) {
	// map内容：endpoint/metric => [strategy1, strategy2 ...]
...
func rebuildExpressionMap(expressionResponse *model.ExpressionResponse) {
	// map内容：metric/tag => [strategy1, strategy2 ...]
