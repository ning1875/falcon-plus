...
func ShouldHandleDevice(device string) bool {
	...
	normalNew := len(device) == 3 && (strings.HasPrefix(device, "df") || strings.HasPrefix(device, "vd"))
	...
	return normal || aws || flash || normalNew
}
...
func GetOnePidIoInfo(pid string) *SingleIOInfo {
	...
	log.Debugln("proc io collect", pid)
	fName := "/proc/" + pid + "/io"
...
func ParseIoLine(line []byte, this *SingleIOInfo) {
	...
	fieldName := fields[0]
	if fieldName == "rchar:" {
		ret, err := strconv.ParseInt(fields[1], 10, 64)
		if err != nil {
			return
		}
		this.Rchar = ret
...
func ProcIoMetrics(name string, pids []string) []*model.MetricValue {
	...
	for _, pid := range pids {
		onePid := GetOnePidIoInfo(pid)
		if onePid == nil {
			log.Debugln("IO proc info get fail:", pid)
			continue
		}
		rcharSum += onePid.Rchar
		wcharSum += onePid.Wchar
	}

	return []*model.MetricValue{
		CounterValue(name+".proc.io.rchar", rcharSum, tag),
		CounterValue(name+".proc.io.wchar", wcharSum, tag),
	}
}
