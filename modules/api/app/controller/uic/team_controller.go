...
func GetTeamDict(c *gin.Context) {
	var (
		dt  *gorm.DB
		err error
	)
...
func UpdateTeam(c *gin.Context) {
	...
	dt := db.Uic
	//log.Printf("user.IsAdmin()", user.IsAdmin())
	//if user.IsAdmin() {
	//	log.Printf("called")
	...
	dt = dt.Raw(`select a.* from team as a, rel_team_user as b 
		where a.id = b.tid AND a.id = ? AND b.uid = ?`,
		cteam.ID, user.ID).Find(&team)
	if dt.Error == nil {
		goto Update
	}

	dt = db.Uic.Raw(`select * from team where creator = ? AND id = ?`,
		user.ID, cteam.ID).Find(&team)
	if dt.Error == nil {
		goto Update
	}

	h.JSONR(c, http.StatusForbidden, "You are neither   admin,nor  team creator, nor  team member ...cannot mangage the team")
	return
