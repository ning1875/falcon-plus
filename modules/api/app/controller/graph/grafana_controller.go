...
func CutEndpointCounterHelp(regexpKey string) (hosts []string, counter string, counterTmp string) {
	r, _ := regexp.Compile("^{?([^#}]+)}?#(.+)")
...
func responseCounterRegexp(regexpKey string) (result []APIGrafanaMainQueryOutputs) {
	result = []APIGrafanaMainQueryOutputs{}
	hosts, counter, _ := CutEndpointCounterHelp(regexpKey)
...
func GrafanaRender(c *gin.Context) {
	...
	for _, target := range inputs.Target {
		hosts, counter, _ := CutEndpointCounterHelp(target)
		//clean characters
		log.Debug(counter)
		re := regexp.MustCompile("\\\\.\\$\\s*$")
