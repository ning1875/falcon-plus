...
func QueryGraphDrawData(c *gin.Context) {
	...
	//log.Println("got_graph_routes_step3_QueryGraphDrawData")
...
func QueryGraphItemPosition(c *gin.Context) {
	var inputs cmodel.GraphInfoParam
	if err := c.Bind(&inputs); err != nil {
		h.JSONR(c, badstatus, err)
		return
	}
	respData, err := grh.Info(inputs)
	if err != nil {
		log.Warn("QueryGraphItemPosition from  graph fail:", err)
	}
	h.JSONR(c, respData)
}
