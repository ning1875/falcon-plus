...
func ScreenCreate(c *gin.Context) {
	...
	var sids []int
	dtdt := db.Dashboard.Table("dashboard_screen").Where("pid = ? and name = ?", ipid, name).Limit(1).Pluck("id", &sids)
	if dtdt.Error != nil || len(sids) > 0 {
		h.JSONR(c, badstatus, fmt.Sprintf("screen_name %v already exists", name))
		return
	}
...
func GetScreens(c *gin.Context) {
	...
	var Screens []*m.DashboardScreen
	var dt *gorm.DB
	if limit != -1 && page != -1 {
		dt = db.Dashboard.Raw(fmt.Sprintf("SELECT * from dashboard_screen  where name regexp '%s' limit %d,%d", q, page, limit)).Scan(&Screens)
	} else {
		dt = db.Dashboard.Table("dashboard_screen").Where("name regexp ?", q).Find(&Screens)
	}
	if dt.Error != nil {
...
func ScreenGetsByName(c *gin.Context) {
	name := c.Params.ByName("s_name")
	if name == "" {
		h.JSONR(c, badstatus, "name is missing")
		return
	}
	screen := m.DashboardScreen{}
	dt := db.Dashboard.Table("dashboard_screen").Where("name = ?", name).Find(&screen)
	if dt.Error != nil {
		h.JSONR(c, badstatus, dt.Error)
		return
	}
