...
import (
	...
	g "github.com/open-falcon/falcon-plus/modules/api/app/model/graph"
...
func DeleteHostGroup(c *gin.Context) {
	...
	if grpID == 0 {
		h.JSONR(c, badstatus, "delete id==0 is not valid")
		return
	}
	user, _ := h.GetUser(c)
...
type APIEndpointGroupTagInputs struct {
	Tag   string `json:"tag" form:"tag"`
	Limit int    `json:"limit" form:"limit"`
	Page  int    `json:"page" form:"page"`
}

func GetGraphEndpointByGroupName(c *gin.Context) {
	inputs := APIEndpointGroupTagInputs{
		//set default is 500
		Limit: 500,
		Page:  1,
	}
	...
	if inputs.Tag == "" {
		h.JSONR(c, http.StatusBadRequest, "tag is all missing")
		return
	}
	...
	portalSql := fmt.Sprintf("select  host.hostname from  host ,grp  ,"+
		"grp_host  where grp_host.grp_id=grp.id and   "+
		"grp_host.host_id=host.id  and grp.grp_name='%s' limit %d offset %d ;", grpName, inputs.Limit, offset)
	var hostNames []f.Host
	dt := db.Falcon.Raw(portalSql).Scan(&hostNames)
	if dt.Error != nil {
		log.Errorf("[dt.Error][portalSql]:%+v", dt.Error)
		h.JSONR(c, http.StatusBadRequest, "portalSql error")
		return
	}
	inStr := ""
	for index, s := range hostNames {
		if index == len(hostNames)-1 {
			inStr += fmt.Sprintf("'%s'", s.Hostname)
		} else {
			inStr += fmt.Sprintf("'%s',", s.Hostname)
		}
	...
	finStr := "(" + inStr + ")"
	//log.Printf("inStr:%s , finStr:%s ", inStr, finStr)
	graphSql := fmt.Sprintf(" select e.endpoint ,e.id   from endpoint e where e.endpoint IN %s", finStr)

	//raw_sql := fmt.Sprintf("select endpoint.endpoint ,endpoint.id  "+
	//		" from falcon_portal.host host  ,  falcon_portal.grp_host grp_host,falcon_portal.grp  grp ,"+
	//		"graph.endpoint endpoint where grp_host.grp_id=grp.id and   "+
	//		"grp_host.host_id=host.id  and   "+
	//		" CONVERT(endpoint.endpoint USING utf8) COLLATE utf8_unicode_ci=host.hostname and  "+
	//		" grp.grp_name like '%s' limit %d offset %d;", grpName, inputs.Limit, offset)
	//log.Println(graphSql)

	var hostgroups []g.Endpoint
	dtG := db.Graph.Raw(graphSql).Scan(&hostgroups)
...
func GetHostGroupSimple(c *gin.Context) {
	grpIDtmp := c.Params.ByName("host_group")
	if grpIDtmp == "" {
		h.JSONR(c, badstatus, "grp id is missing")
		return
	...
	raw_sql := fmt.Sprintf("select host.hostname from host  ,  grp_host ,grp   where grp_host.grp_id=grp.id and  "+
		" grp_host.host_id=host.id  and   grp_host.grp_id='%d';", grpID)

	var hostlist []string
	var hostgroups []f.Host
	dt := db.Falcon.Raw(raw_sql).Scan(&hostgroups)

	if dt.Error != nil {
		log.Error(dt.Error)
	}
	for _, i := range hostgroups {
		//fmt.Println(i)
		hostlist = append(hostlist, i.Hostname)
	}
...
func GetHostGroupHosts(c *gin.Context) {
	grp_name := c.Params.ByName("host_group")
	if grp_name == "" {
		h.JSONR(c, badstatus, "grp name is missing")
		return
	}

	raw_sql := fmt.Sprintf("select hostname from host,grp_host,grp "+
		"where host.id=grp_host.host_id and grp_host.grp_id=grp.id and grp.grp_name='%s';", grp_name)
