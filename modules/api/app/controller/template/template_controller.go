...
func GetActionByID(c *gin.Context) {
	...
	//TODO 暂时测试，DB中缺少lark_group_id这个字段
	//act.LarkGroupId = "6581614934021374222"

	h.JSONR(c, act)
}

func GetHostGroupByTplidAndEndpoint(c *gin.Context) {
	tplid := c.Query("tpl_id")
	endpoint := c.Query("endpoint")
	log.Infof("tplid,endpoint", endpoint, tplid)
	var hostgroupNames []f.HostGroups
	tplId, err := strconv.Atoi(tplid)
	if err != nil {
		h.JSONR(c, badstatus, "invalid tpl_id id")
		return
	}
	sql := fmt.Sprintf("SELECT a.grp_id,d.grp_name from grp_tpl a , grp_host b,host c,grp d where a.grp_id=b.grp_id and d.id=a.grp_id and a.tpl_id=%d and b.host_id=c.id and c.hostname='%s'", tplId, endpoint)
	dt := db.Falcon.Raw(sql).Scan(&hostgroupNames)
	if dt.Error != nil {
		h.JSONR(c, badstatus, dt.Error)
		return
	}

	h.JSONR(c, hostgroupNames)
}
