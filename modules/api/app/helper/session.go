...
func SessionChecking(c *gin.Context) (auth bool, err error) {
	...
	sig_redis_key := fmt.Sprintf("user_sig_%s", websessio.Name)
	value, err := config.RedisGet(sig_redis_key)
	if err != nil {
		log.Error("get_sig_from_redis_error", err)
		return
	}
	...
	if session.Sig == websessio.Sig {
		auth = true
		return
	} else {
		err = errors.New(fmt.Sprintf("Sig_wrong:%s", websessio.Sig))
		return
	}
