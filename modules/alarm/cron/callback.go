...
func HandleCallback(event *model.Event, action *api.Action, amstag string, grps string) {
	...
	if teams != "" {
		userNames, mails = api.ParseTeams(teams)
		...
		if action.BeforeCallbackSms == 1 {
			redi.WriteSms(userNames, smsContent)
			redi.WriteIM(userNames, imContent)
		}
	...
	message := Callback(event, action, grps)

	if teams != "" {
		if action.AfterCallbackSms == 1 {
			redi.WriteSms(userNames, message)
			redi.WriteIM(userNames, message)
		}
