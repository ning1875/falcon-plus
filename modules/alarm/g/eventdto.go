...
func ModLink(event *cmodel.Event) string {
	tplId := event.TplId()
	if tplId != 0 {
		return fmt.Sprintf("%s/portal/template/update/%d", Config().Api.Dashboard, tplId)
	}

	eid := event.ExpressionId()
	if eid != 0 {
		return fmt.Sprintf("%s/portal/expression/update/%d", Config().Api.Dashboard, eid)
	}
...
func ModUicLink(uic string) string {
	return fmt.Sprintf("%s/team/%s/edit", Config().Api.Dashboard, uic)
